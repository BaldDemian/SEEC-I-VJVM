...
public class FieldInfo {
  ...
  public FieldInfo(DataInput dataInput, JClass jClass) {
    ...
    name = ((UTF8Constant)this.jClass.constantPool().constant(nameIndex)).value();
    int descriptorIndex = dataInput.readUnsignedShort();
    descriptor = ((UTF8Constant)this.jClass.constantPool().constant(descriptorIndex)).value();
    int attributeCount = dataInput.readUnsignedShort();
    attributes = new Attribute[attributeCount];
    for (int i = 0; i < attributeCount; i++) {
      attributes[i] = Attribute.constructFromData(dataInput, jClass.constantPool(), null);
    }
  }

  @Override
  public String toString() {
    return name + (String.format("(0x%x)", accessFlags)) + ": " + descriptor;
  }
